...
public class BruteCollinearPoints {
    ...
    public BruteCollinearPoints(Point[] points) {
        ...
        Arrays.sort(points); // sort the points for proper processing
        for (int i = 0; i < N; i++){
            for (int j = i+1; j < N; j++){
                // check for duplicate points (corner case) here as opposed to at beginning
                if (points[i].compareTo(points[j]) == 0) throw new IllegalArgumentException("duplicate points found");
                for (int k = j+1; k < N; k++){
                    for (int l = k+1; l < N; l++){
                        p = points[i]; q = points[j]; r = points[k]; s = points[l];
